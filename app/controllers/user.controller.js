...
exports.createUser = (req, res) => {
  ...
  User.create(user)
    ...
    .catch(err => {
      res.status(500).send({
        message: err.message || "Algún error ha ocurrido en la creación del usuario."
      });
    });
};
  ...
  User.findByPk(req.params.id, {
    ...
    .then(user => {
      if (user) {
        ...
        res.status(404).send({
          message: "Algún error ha ocurrido mientras se buscaba al usuario."
        });
      }
    })
    .catch(err => {
...
exports.findAll = (req, res) => {
  return User.findAll({
    ...
    .catch(err => {
      res.status(404).send({
        message: err.message || "Algún error ha ocurrido mientras se buscaba al usuario."
      });
    })
}
...
exports.updateUserById = (req, res) => {
  ...
  return User.update({
    ...
    .catch(err => {
      res.status(500).send({
        message: err.message || "Algun error ha ocurrido mientras se actualizaba el usuario."
      });
    })
}
