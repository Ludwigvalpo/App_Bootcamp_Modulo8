...
exports.createBootcamp = (req, res) => {
  ...
  return Bootcamp.create({
    ...
    .catch(err => {
      res.status(500).send({
        message: err.message || "Algún error ha ocurrido en la creación del bootcamp."
      });
    });
}
  ...
  return Bootcamp.findByPk(req.params.id, {
    ...
    .then(bootcamp => {
      if (bootcamp) {
        ...
        res.status(404).send({
          message: "Algún error ha ocurrido mientras se buscaba el bootcamp."
        });
      }
    })
    .catch(err => {
...
exports.findAll = (req, res) => {
  return Bootcamp.findAll({}).then(bootcamp => {
    return res.send(bootcamp)
  }).catch((err) => {
    res.status(404).send({
